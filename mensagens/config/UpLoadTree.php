...
class Upload{
	...
	public $diretorio = "ficheirosrecebidos/";					
	
	//1 mega = 1000000, 2 mega = 2000000		
	public $tamanho   = 10000000;
	
	//Extencao que nao é permitida no programa          						
	public $extencao  = array('a','xls','sf','exe');	
	...
	function verificaTamanhoArquivo($arquivo , $tamanho , $tmp_nome , $tipo){
		if($tamanho > $this->tamanho){
			echo "<script>alert('O ficheiro é demasiado grande.');</script>";
			return false;
		}else{			
			...
			if($retorno == 1){
				...
				echo "<script>alert('Extensão do ficheiro inválida.');</script>";	
				return 0;
			}
		}	
	}
